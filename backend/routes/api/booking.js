...
router.put('/:id', requireAuth, validateBooking, async(req,res)=>{
  ...
  console.log(newEnd)
          ...
          for(let i=0; i<findBookings.length; i++){
            ...
            if(findBookings[i].id === number)
            {
                console.log(findBookings[i], 'before')
                findBookings.splice(1,i)
                console.log(findBookings[i],'after')
                newBooks = findBookings
            }
        }

        for(let i=0; i<newBooks.length; i++){
            if(newBooks[i].startDate >= newStart && newBooks[i].endDate <= newEnd){
                res.status(403).json({ message: "Sorry, this spot is already booked for the specified dates",
                statusCode: 403,
                errors: [
                  "Start date conflicts with an existing booking",
              ...
              if(newStart >= newBooks[i].startDate && newStart <=newBooks[i].endDate){
                res.status(403).json({ message: "Sorry, this spot is already booked for the specified dates",
                statusCode: 403,
                errors: [
                    "Start date conflicts with an existing booking"]
              ...
              if(newEnd >= newBooks[i].startDate && newStart <=newBooks[i].endDate){
                console.log(newEnd)
                console.log(findBookings[i].startDate)
                console.log(newStart)
                console.log(findBookings[i].endDate)
